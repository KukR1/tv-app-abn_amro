...
<template>
  <article
    class="group relative w-36 min-w-36 overflow-hidden rounded-xl border border-slate-700 bg-slate-900 transition duration-200 hover:border-slate-300 hover:shadow-[0_10px_30px_rgba(15,23,42,0.6)] sm:w-40 sm:min-w-40 md:w-48 md:min-w-48"
  ...
  >
    ...
    <div class="absolute inset-0 bg-linear-to-t from-slate-950/80 via-slate-950/30 to-transparent opacity-80" />
    ...
    <div class="absolute inset-x-0 bottom-0 p-3 transition duration-200 group-hover:opacity-0 group-focus-within:opacity-0">
      ...
      <p class="mt-1 text-xs text-slate-200">{{ showGenres }}</p>
    </div>

    <div
      class="pointer-events-none absolute inset-x-0 bottom-0 translate-y-3 rounded-t-lg bg-slate-900/75 p-3 opacity-0 backdrop-blur-[1px] transition duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100"
    >
      <div class="w-full space-y-2 text-xs text-slate-200">
        <div class="flex items-center justify-between gap-2">
          <p class="min-w-0 truncate pr-1 text-sm font-semibold text-white">{{ show.name }}</p>
          <span class="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-slate-500 bg-slate-800/80 text-white" aria-hidden="true"> ▶ </span>
        </div>
