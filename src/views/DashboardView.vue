...
<template>
  <section class="relative flex flex-col gap-4">
    ...
    <div class="transition duration-900" :class="isSearchFocusMode ? 'pointer-events-none select-none opacity-25 blur-[3px] saturate-50' : 'opacity-100 blur-0'">
      <p v-if="isLoading" class="text-teal-300">Loading shows...</p>
      <p v-else-if="errorMessage" class="text-red-300">{{ errorMessage }}</p>

      <div v-else class="grid gap-5">
        <section v-for="group in genreGroups" :key="group.genre" class="grid gap-2.5 overflow-visible">
          <h2 class="m-0 text-xl font-semibold text-white">{{ group.genre }}</h2>
